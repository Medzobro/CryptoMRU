...
const translations = {
    en: {
        ...
        loading: "Loading...",
        error_msg: "Failed to load data. Retrying...",
        error_btc: "BTC price unavailable",
        error_mru: "MRU rate unavailable",
        success: "Updated",
        error: "Error"
    },
    ar: {
        ...
        error_msg: "فشل تحميل البيانات. جاري إعادة المحاولة...",
        error_btc: "سعر البيتكوين غير متاح",
        error_mru: "سعر الصرف غير متاح",
        success: "تم التحديث",
        error: "خطأ"
    },
    fr: {
        ...
        loading: "Chargement...",
        error_msg: "Échec du chargement. Nouvelle tentative...",
        error_btc: "Prix BTC indisponible",
        error_mru: "Taux MRU indisponible",
        success: "Mis à jour",
        error: "Erreur"
    }
};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const savedLang = localStorage.getItem('cryptomru_lang');
    ...
    fetchPrices();
    // Auto refresh every 30 seconds (to respect API limits)
...
async function fetchPrices() {
    ...
    let mruSuccess = false;

    try {
        // API 1: CoinGecko for BTC/USD with API key in headers
        ...
        if (!btcRes.ok) {
            throw new Error(`CoinGecko error: ${btcRes.status}`);
        }
        const btcData = await btcRes.json();
        btcPriceUSD = btcData.bitcoin.usd;
        btcSuccess = true;
        
        // Update BTC display with 4 decimal places
        animateValue(els.btcUsd, btcPriceUSD, "$", 4);
        
    } catch (error) {
        console.error("BTC API Error:", error);
        if (!btcSuccess) {
            els.btcUsd.innerText = t.error_btc;
        }
    }
    ...
    try {
        ...
        if (!mruRes.ok) {
            throw new Error(`ExchangeRate error: ${mruRes.status}`);
        }
        const mruData = await mruRes.json();
        usdToMru = mruData.conversion_rates.MRU;
        mruSuccess = true;
        
        // Update MRU display with 6 decimal places for accuracy
        animateValue(els.usdMru, usdToMru, "MRU", 6);
        
    } catch (error) {
        console.error("MRU API Error:", error);
        if (!mruSuccess) {
            els.usdMru.innerText = t.error_mru;
        }
    }

    // Check overall success
    if (btcSuccess && mruSuccess) {
        isDataLoaded = true;
        fetchAttempts = 0;
        ...
        if (fetchAttempts < MAX_RETRIES) {
            fetchAttempts++;
            updateStatus(`${t.error_msg} (${fetchAttempts}/${MAX_RETRIES})`);
            setTimeout(fetchPrices, 5000); // Retry after 5 seconds
        } else {
...
// --- UI UPDATES ---
function animateValue(element, value, prefix, decimals) {
    if (!element) return;
    ...
    element.innerText = `${prefix} ${formatted}`;
}
...
function updateStatus(msg) {
    ...
    els.lastUpdated.innerText = `${prefix} ${timeString} - ${msg}`;
}
...
function setLanguage(lang) {
    ...
    // Save preference
    localStorage.setItem('cryptomru_lang', lang);
    ...
    // Update price displays if data loaded, else show loading
    if (isDataLoaded) {
        animateValue(els.btcUsd, btcPriceUSD, "$", 4);
        animateValue(els.usdMru, usdToMru, "MRU", 6);
    } else {
    ...
    document.querySelectorAll('.lang-selector button').forEach(btn => {
        ...
        if ((lang === 'en' && btn.innerText.includes('🇬🇧')) ||
            (lang === 'ar' && btn.innerText.includes('🇦🇪')) ||
            (lang === 'fr' && btn.innerText.includes('🇫🇷'))) {
